...
export class WebSocketManager {
  ...
  private agentResponseQueue: Map<
    number,
    Array<{ messageId: number; agentId: number }>
  > = new Map();
  ...
  private setupWebSocketServer(): void {
    this.wss.on("connection", (ws: WebSocket, request) => {
      ...
      const agentId = url.searchParams.get("agentId")
        ? parseInt(url.searchParams.get("agentId")!)
        : undefined;
  ...
  private async handleMessage(clientId: string, data: Buffer): Promise<void> {
    ...
    try {
      ...
      console.log(
        `[WebSocket] Received message type: ${message.type}, from client: ${clientId}, content: "${message.content}"`,
      );
  ...
  private async handleUserMessage(
    ...
    if (!message.content || !message.conversationId) {
      console.log(
        `[WebSocket] Skipping message - content: "${message.content}", conversationId: ${message.conversationId}`,
      );
      return;
    }

    console.log(
      `[WebSocket] Processing user message from user ${client.userId}: "${message.content}" in conversation ${message.conversationId}`,
    );
    ...
    try {
      ...
      console.log(
        `[WebSocket] User message stored with ID: ${storedMessage.id}`,
      );
  ...
  private async triggerAgentResponses(
    ...
    console.log(
      `[WebSocket] Starting triggerAgentResponses for conversation ${conversation.id}`,
    );

    if (!conversation.participants) {
      console.log(
        `[WebSocket] No participants found in conversation ${conversation.id}`,
      );
      return;
    }
    ...
    console.log(
      `[WebSocket] Found ${agentIds.length} agents in conversation ${conversation.id}:`,
      agentIds,
    );
    console.log(
      `[WebSocket] All agents available:`,
    ...
    for (const agentId of agentIds) {
      ...
      try {
        ...
        console.log(
          `[WebSocket] Agent ${agentId} should respond: ${shouldRespond}`,
        );

        if (shouldRespond) {
          console.log(
            `[WebSocket] Agent ${agentId} will respond to message: "${content}"`,
          );
          this.queueAgentResponse(conversation.id, userMessage.id, agentId);
        }
      } catch (error) {
        console.error(
          `[WebSocket] Error checking if agent ${agentId} should respond:`,
          error,
        );
      }
    }
  ...
  async triggerAgentResponsesFromAPI(
    conversation: any,
    userMessage: any,
    content: string,
  ): Promise<void> {
    console.log(
      `[REST API] Triggering agent responses for conversation ${conversation.id}, participants:`,
    ...
    try {
      ...
      console.log(
        `[REST API] Agent response triggering completed for conversation ${conversation.id}`,
      );
    } catch (error) {
  ...
  private async shouldAgentRespond(
    ...
    const mentionedByName = content
      .toLowerCase()
      .includes(agent.name.toLowerCase());
    const mentionedByRole = content
      .toLowerCase()
      .includes(agent.type.replace("_", " "));
    ...
    const isCollaborativeMessage =
      content.toLowerCase().includes("team") ||
      content.toLowerCase().includes("discuss") ||
      content.toLowerCase().includes("together");
    ...
    console.log(
      `[Agent ${agentId}] ${agent.name} shouldRespond: ${shouldRespond} (mentioned: ${mentionedByName}, role: ${mentionedByRole}, question: ${isQuestion}, keywords: ${hasKeywords}, chance: ${responseChance})`,
    );
  ...
  private hasRelevantKeywords(content: string, agent: any): boolean {
    ...
    const keywords =
      agentKeywords[agent.type as keyof typeof agentKeywords] || [];
    return keywords.some((keyword: string) =>
      lowercaseContent.includes(keyword),
    );
  }

  private queueAgentResponse(
    conversationId: number,
    messageId: number,
    agentId: number,
  ): void {
  ...
  private async processAgentResponseQueue(
    conversationId: number,
  ): Promise<void> {
    const queue = this.agentResponseQueue.get(conversationId);
    console.log(
      `[Queue] Processing queue for conversation ${conversationId}, queue length: ${queue?.length || 0}`,
    );

    if (!queue || queue.length === 0) {
      console.log(
        `[Queue] No items in queue for conversation ${conversationId}`,
      );
      return;
    }
    ...
    console.log(
      `[Queue] Processing response for agent ${agentId}, message ${messageId}`,
    );
    ...
    try {
      ...
      if (!conversation || !userMessage) {
        console.error(
          `[Queue] No conversation or user message found for agent ${agentId} in conversation ${conversationId}`,
        );
        return;
      }

      console.log(
        `[Queue] Agent ${agentId} is responding to: "${userMessage.content}"`,
      );
      ...
      try {
        ...
        console.log(
          `[Queue] Agent ${agentId} generated response:`,
          response.content?.substring(0, 100),
        );
      } catch (error) {
        console.error(
          `[Queue] Error generating agent response for agent ${agentId}:`,
          error,
        );
        // Fallback response
        const agent = await storage.getAgent(agentId);
  ...
  private async autoJoinUserConversations(
    ...
    try {
      ...
      const conversations = await storage.getConversationsByParticipant(
        client.userId,
      );
  ...
  private startPeriodicTasks(): void {
    ...
    setInterval(() => {
      const now = new Date();
      this.typingUsers.forEach((status, key) => {
        if (now.getTime() - status.timestamp.getTime() > 10000) {
          // 10 seconds
          this.typingUsers.delete(key);
        }
      });
