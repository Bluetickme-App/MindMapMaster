...
class OpenAIProvider implements AIProvider {
  ...
  async generateResponse(
    ...
    try {
      ...
      throw new Error(
        `OpenAI provider failed: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
  ...
  async generateStructuredResponse(
    ...
    try {
      ...
      throw new Error(
        `OpenAI structured response failed: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
  ...
  private calculateConfidence(choice: any): number {
    ...
    if (
      ...
      choice.message.content.length > 50
    ) {
      return 0.9;
...
class AnthropicProvider implements AIProvider {
  ...
  async generateResponse(
    ...
    try {
      ...
      throw new Error(
        `Anthropic provider failed: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
...
Please respond with valid JSON that matches this schema: ${JSON.stringify(schema)}`;
      ...
      throw new Error(
        `Anthropic structured response failed: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
...
class GeminiProvider implements AIProvider {
  ...
  async generateResponse(
    ...
    try {
      ...
      throw new Error(
        `Gemini provider failed: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
  ...
  async generateStructuredResponse(
    ...
    try {
      ...
      throw new Error(
        `Gemini structured response failed: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
...
export class MultiAIProviderService {
  ...
  constructor() {
    ...
    this.providers.set("codex", new OpenAIProvider()); // Add codex alias for OpenAI
  ...
  async generateResponseWithFallback(
    ...
    try {
      ...
      console.warn(
        `Primary provider ${preferredProvider} failed, trying fallbacks:`,
        error,
      );
    }
  ...
  async generateStructuredResponse(
    ...
    return aiProvider.generateStructuredResponse(
      prompt,
      systemPrompt,
      schema,
      model,
  ...
  async generateConsensusResponse(
    ...
    for (const provider of providers) {
      try {
        ...
        console.warn(
          `Provider ${provider} failed in consensus generation:`,
          error,
        );
      }
    }
  ...
  private async synthesizeConsensus(
    ...
    try {
      ...
      console.warn(
        "Consensus synthesis failed, using best single response:",
        error,
      );
      return responses[0].content;
    }
  ...
  getRecommendedModel(agentType: string, provider: string): string {
    ...
    return (
      recommendations[agentType]?.[provider] || this.getDefaultModel(provider)
    );
  }
...
Respond with a JSON object containing:
{
  ...
  async analyzeError(
    errorMessage: string,
    command: string,
  ): Promise<{ analysis: string; suggestions: string[]; fixes: string[] }> {
...
Please analyze this error and provide actionable solutions.`;
    ...
    try {
      ...
      return {
        analysis:
          "Error analysis failed. Please check the error message manually.",
        suggestions: [
